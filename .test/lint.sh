...
lint_driver () {
    pushd $SRC_DIR/drivers/rtmouse
        python3 $SRC_DIR/.test/bin/run-clang-format.py rtmouse.c
    popd
}

check_driver_version () {
    MOD_VER=$(grep MODULE_VERSION $SRC_DIR/drivers/rtmouse/rtmouse.c | sed -E 's/MODULE_VERSION\("([0-9.]*)"\);/\1/g')
    COMMENT_VER=$(grep '* Version' $SRC_DIR/drivers/rtmouse/rtmouse.c | sed -E 's/.*Version: ([0-9.]*)/\1/g')
